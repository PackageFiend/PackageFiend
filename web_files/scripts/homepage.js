...
$(document).ready(() => {
  if (localStorage.pkgfnd_name) {
    $('.sign_in_box').html('<a href="https://packagefiend.com/track/dashboard">Dashboard</a> Hello, ' + localStorage.pkgfnd_name);
  }
  ...
  $('.go_button').click(() => {
    ...
    if (tnums !== "" && !err) {
      window.location = `https://packagefiend.com/track/num/${tnums.replace(/\s/g, '')}`;
    }
  });
});
