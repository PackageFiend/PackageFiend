...
function send_to_tracking() {
  ...
  if (tnums !== "" && !err) {
    window.location = `http://www.packagefiend.com/track/num/${tnums.replace(/\s/g, '')}`;
  }
}
...
$(document).ready(() => {
  ...
  if (localStorage.pkgfnd_name) {
    ...
    const user_html = `<a id="dashboard_link" href="http://www.packagefiend.com/track/dashboard/">DASHBOARD</a><div class="user_icon"><div class="user_initial">${user_initial}</div><div class="logout_button">Logout</div></div>`;
    $('.sign_in_box').append(user_html);
  } else {
    $('.sign_in_box').append('<a href="http://www.packagefiend.com/signin.html"><div class="sign_in_button">Sign In</div></a>');
  };
